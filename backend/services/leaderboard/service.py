...
class Leaderboard:
    def __init__(self, irsdk_service, parser):
    ...
    @staticmethod
    def normalize_laps_completed(laps: int) -> int:
        """Заменяет -1 на 0 для удобного отображения"""
        return 0 if laps < 0 else laps

    @staticmethod
    def normalize_total_laps(total: int) -> int:
        """32767 (безлимит) → 0, иначе как есть"""
        return 0 if total == 32767 else total

    def get_leaderboard_snapshot(self):
        """Внутренняя функция для получения снимка лидерборда без сохранения в файл"""
        ...
        def build_car(idx: int):
            ...
            laps = self.normalize_laps_completed(int(safe_get(laps_completed, idx, 0) or 0))
        ...
        player_data = player_car or {
            ...
            "laps_completed": self.normalize_laps_completed(int(safe_get(laps_completed, player_idx, 0) or 0)),
            "irating": None,
            "license": None,
            "lap_dist_pct": None,
        }
        ...
        leaderboard_data = {
            "total_laps": self.normalize_total_laps(int(total_laps)),
        }
